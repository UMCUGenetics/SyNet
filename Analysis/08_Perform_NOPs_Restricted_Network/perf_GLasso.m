function result = perf_GLasso(dataset_info, opt_info)
...
cTr = zeros(n_Trsample, n_snet*MAX_SUBNET_SIZE);
cTe = zeros(n_Tesample, n_snet*MAX_SUBNET_SIZE);
group_ind = ones(3, n_snet);
step = 1;
for si=1:n_snet
	group_ind(1, si) = step;
	for gi=1:numel(SubNet_Trimmed{si})
		cTr(:, step) = zTr(:, SubNet_Trimmed{si}(gi));
		cTe(:, step) = zTe(:, SubNet_Trimmed{si}(gi));
		step = step + 1;
	end
