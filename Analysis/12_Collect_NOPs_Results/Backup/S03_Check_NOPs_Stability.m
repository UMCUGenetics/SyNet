...
%% Load NOPs results
load('./Collected_NOP_CV01_Markers.mat', 'Gene_Name', 'Result_Marker', 'method_lst', 'net_lst', 'Result_AUC');
...
for mi=1:n_met
	for ni=1:n_net
		...
		for si=1:n_study
			...
			Top_Gene = nonzeros(Mrk_set(:,1:100));
	...
	for ni=1:n_net
		for si=1:n_study
			for sj=si+1:n_study
				...
				Study_fet{mi,ni} = nonzeros([Study_fet{mi,ni} -log10(pval)])';
			end
		end
	end
end
imagesc(cellfun(@median, Study_fet));
set(gca, 'XTick', 1:numel(net_lst), 'XTickLabel', net_lst, 'XTickLabelRotation', 30, ...
	'YTickLabel', method_lst, 'CLim', [50 200]);
...
for ni=[1 2 3 5]
	...
	text(step-2, 250, net_lst{ni}, 'HorizontalAlignment', 'Center', 'VerticalAlignment', 'Top', ...
...
%% Saving
% output_name = sprintf('./Plots/S03_NOPs_StabilityFET.pdf');
