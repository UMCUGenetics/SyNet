...
for ni=1:n_net
    for mi=1:n_met
        for ri=1:n_rep
            res_ptr = sprintf([result_path 'DID_CVT01_Si%02d-Ri%03d_%s_*_MSN-500_MTN-%s.mat'], study_index, ri, Net_lst{ni}, Met_lst{mi});
...
%% Save
out_name = sprintf([sav_path 'SingleStudy_Si%02d_Results.mat'], study_index);
...
figure();
study_index = 12;
res_name = sprintf('./Collected_Results/SingleStudy_Si%02d_Results.mat', study_index);
...
title(sprintf('Performance of Group Lasso for [%s] study', GE_Info.Study_Name{study_index}));
clrbar_h = colorbar();
...
%% Saving
output_name = sprintf('./Plots/S12_GLasso_SingleStudyPerf_Si%02d.pdf', study_index);
