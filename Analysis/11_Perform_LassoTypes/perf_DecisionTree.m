function result = perf_DecisionTree(dataset_info, opt_info)
    ...
    for li=1:n_LeafSize
        ...
        for fi=1:n_Fold
            ...
            try
                ...
                DT_Model = fitctree(zTr(iTr,:), lTr(iTr), 'MinLeafSize', opt_info.MinLeafSize(li), ...
                    'Weights', Sample_Weights, 'Surrogate','off');
                [~, pred_prob] = predict(DT_Model, zTr(iTe,:));
...
DT_Model = fitctree(zTr, lTr, 'MinLeafSize', opt_LeafSize, ...
                    'Weights', Sample_Weights, 'Surrogate','off');
                
%% Evaluating the model
fprintf('Evaluating the model ...\n');
tr_auc = 1;
[~, pred_prob] = predict(DT_Model, zTe);
[~, pred_lbl] = max(pred_prob, [], 2);
