function result = perf_LDA(dataset_info, opt_info)
...
%% Training classifier
fprintf('Training the LDA over [%d] features...\n', n_gene);
[pred, ~] = lda(zTr, lTr, zTe, opt_info);
...
function [label, score] = lda(xTr, lTr, zTe, lda_opt)
Sample_Weights = (lTr==-1)/sum(lTr==-1) + (lTr==1)/sum(lTr==1);
LDA_Mdl = fitcdiscr(xTr, lTr, 'DiscrimType', 'linear', 'Weights', Sample_Weights);
[label, score, Cost] = predict(LDA_Mdl, zTe);
end
