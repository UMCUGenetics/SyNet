...
net_lst = {
	'Random-T00500', 'Random-T00200', 'Random-T00100', ...
	'Random-T00050', 'Random-T00020', 'Random-T00010'
	};
n_net = numel(net_lst);
n_study = 1;
n_rep = 50;
...
%% Main loop
Result_AUC = nan(n_met, n_net, n_rep);
for mi=1:n_met
	for ni=1:n_net
		Method_Name = method_lst{mi};
		Net_Name = net_lst{ni};
		sav_name = sprintf([sav_path 'CRES_CVT91_%s_%s.mat'], Method_Name, Net_Name);
		if exist(sav_name, 'file')
			...
			for si=1:n_study
				for ri=1:n_rep
					res_name = sprintf('%sDID_CVT91_Si%02d-Ri%03d_%s_*_MTN-%s.mat', result_path, si, ri, Net_Name, Method_Name);
