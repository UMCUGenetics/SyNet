...
%% Load Top pairs
Pair_Info = collectTopPairs(ge_name);
...
close all
figure();
plot(NP_Info(:,6), NP_Info(:,7), '.', 'Color', [0.8 0.8 0.8], 'MarkerEdgeColor', [0.8 0.8 0.8], 'MarkerSize', 1);
hold on
plot(PP_Info(:,6), PP_Info(:,7), '.', 'Color', [0.3 0.8 0.2], 'MarkerEdgeColor', [0.3 0.8 0.2], 'MarkerSize', 4);

[hit_map, center] = hist3([NP_Info(:,6), NP_Info(:,7)], 'Nbins', [50 50]);
...
x_lim = [min(NP_Info(:,6))*0.98 max(NP_Info(:,6))*1.02];
y_lim = [min(NP_Info(:,7))*0.98 max(NP_Info(:,7))*1.02];
xlim(x_lim);
ylim(y_lim);
...
function Pair_Info = collectTopPairs(ge_name)
%% Load gene name
