function S01_Matlab_Study_Test(Train_si, Test_si)
%% Run
% si=1;sj=2;sbatch --job-name=Test-S${si}-S${sj} --mem=8GB --partition=general --qos=short --ntasks=1 --cpus-per-task=1 --time=4:00:00 --output=Logs/Log_Test_S${si}_S${sj}_%J_%a-%N.out run_Matlab.sh S01_Matlab_Study_Test $si,$sj
...
[~, ~] = mkdir(sav_path);
sav_name = sprintf('%s/LassoEx_%s_%s.mat', sav_path, Study_lst{Train_si}, Study_lst{Test_si});
fprintf('Saving in [%s]\n', sav_name);
