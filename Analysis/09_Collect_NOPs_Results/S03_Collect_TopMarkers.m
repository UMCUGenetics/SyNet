...
for mi=1:n_met
	for ni=1:n_net
		sav_name = sprintf([sav_path 'MRK_CVT01_%s_%s_MSN-%03d.mat'], method_lst{mi}, net_lst{ni}, MAX_N_SN);
		if exist(sav_name, 'file')
			...
			for si=1:n_study
				for ri=1:n_rep
					res_name = sprintf('%sDID_CVT01_Si%02d-Ri%03d_%s_*_MSN-%03d_MTN-%s.mat', result_path, si, ri, net_lst{ni}, MAX_N_SN, method_lst{mi});
...
save('./Collected_NOP_Markers.mat', 'Result_Marker', 'Result_AUC', 'Gene_Name', 'net_lst', 'method_lst');
